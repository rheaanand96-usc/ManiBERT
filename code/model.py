...
class VisualBERT():
    ...
    def get_embeddings(self, text_embeds, visual_embeds):
        model = VisualBertForRegionToPhraseAlignment.from_pretrained('uclanlp/visualbert-vqa-coco-pre')
        visual_token_type_ids = torch.ones(visual_embeds.shape[:-1], dtype=torch.long)
        visual_attention_mask = torch.ones(visual_embeds.shape[:-1], dtype=torch.float)
        region_to_phrase_position = torch.ones((1, text_embeds["input_ids"].shape[-1] + visual_embeds.shape[-2]))
        text_embeds.update({
        ...
        with torch.no_grad():
            ...
            for sent in captions:
                ...
                for word in sent.split():
                    ...
                    word_embedding = self.get_word_vector(sent, idx, tokenizer, model, layers)
...
class NEXT_STAGE_G(nn.Module):
    ...
    def define_module(self):
        ...
        if cfg.USE_VISBERT == True:
            self.SAIN = VisualBERT()
        else:
            self.SAIN = ACM(ngf * 3)
    ...
    def forward(self, h_code, c_code, word_embs, mask, img):
        ...
        if cfg.USE_VISBERT == True:
            h_c_c_img_code = self.SAIN.get_embeddings(h_c_c_code, img)
        else:
...
class G_NET(nn.Module):
    ...
    def forward(self, z_code, sent_emb, word_embs, mask, cnn_code, region_features):
        ...
        if cfg.TREE.BRANCH_NUM > 0:
            ...
            if cfg.USE_VISBERT == True:
                h_code_img1 = self.SAIN1.get_embeddings(h_code1, img_code64)
            else:
        ...
        if cfg.TREE.BRANCH_NUM > 1:
            ...
            if cfg.USE_VISBERT == True:
                h_code_img2 = self.SAIN2.get_embeddings(h_code2, img_code128)
            else:
        ...
        if cfg.TREE.BRANCH_NUM > 2:
            ...
            if cfg.USE_VISBERT == True:
                h_code_img3 = self.SAIN3.get_embeddings(h_code3, img_code256)
            else:
...
class DCM_Net(nn.Module):
    ...
    def forward(self, x, real_features, sent_emb, word_embs, mask, c_code):
        ...
        if cfg.USE_VISBERT == True:
            h_a_r_code = self.SAIN.get_embeddings(h_a_code, r_code)
        else:
